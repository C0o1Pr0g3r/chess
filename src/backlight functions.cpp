...
inline void SelectedFigureBacklight(AppState& appState, int x, int y)
{
    ...
    if (PieceIsChoose)
    {
        GreenSquare_sprite.SetPosition({static_cast<float>(x * CFDCP), static_cast<float>(y * CFDCP)});
...
void BacklightOfPossiblePawnMovements(AppState& appState, int OpponentColor, int ox, int oy)
{
    ...
    {
        DarkCircle_sprite.SetPosition({static_cast<float>(ox * CFDCP), static_cast<float>((oy + step1) * CFDCP)});
    ...
    {
        DarkCircle_sprite.SetPosition({static_cast<float>(ox * CFDCP), static_cast<float>((oy + step2) * CFDCP)});
        DarkCircle_sprite.Draw(window);
    }
    if (board[oy + step1][ox - 1] > 0 && FIGURE_COLOR(board[oy + step1][ox - 1]) == OpponentColor && IsAllowedMF(appState, ox, oy, ox - 1, oy + step1))
    {
        YellowSquare_sprite.SetPosition({static_cast<float>((ox - 1) * CFDCP), static_cast<float>((oy + step1) * CFDCP)});
    ...
    {
        DarkCircle_sprite.SetPosition({static_cast<float>(PawnOnAisleCoordinates.x * CFDCP), static_cast<float>((oy + step1) * CFDCP)});
        DarkCircle_sprite.Draw(window);
    }
}
...
void BacklightOfPossibleRookMovements(AppState& appState, int OpponentColor, int ox, int oy)
{
    ...
    for (i = oy - 1; i > TOP_EXTREME_COORDINATE - 1; i--)   //up
    {
        if (board[i][ox] > 0)
        {
            ...
            {
                YellowSquare_sprite.SetPosition({static_cast<float>(ox * CFDCP), static_cast<float>(i * CFDCP)});
                YellowSquare_sprite.Draw(window);
            }
            break;
        }
        ...
        {
            DarkCircle_sprite.SetPosition({static_cast<float>(ox * CFDCP), static_cast<float>(i * CFDCP)});
            DarkCircle_sprite.Draw(window);
        }
    }
    ...
    for (i = ox - 1; i > LEFT_EXTREME_COORDINATE - 1; i--)   //left
    {
        if (board[oy][i] > 0)
        {
            ...
            {
                YellowSquare_sprite.SetPosition({static_cast<float>(i * CFDCP), static_cast<float>(oy * CFDCP)});
                YellowSquare_sprite.Draw(window);
            }
            break;
        }
        ...
        {
            DarkCircle_sprite.SetPosition({static_cast<float>(i * CFDCP), static_cast<float>(oy * CFDCP)});
            DarkCircle_sprite.Draw(window);
        }
    }
...
void BacklightOfPossibleBishopMovements(AppState& appState, int OpponentColor, int ox, int oy)
{
    ...
    for (i = ox + 1, j = oy - 1; i < RIGHT_EXTREME_COORDINATE + 1 && j > TOP_EXTREME_COORDINATE - 1; i++, j--)
    {
        if (board[j][i] > 0)
        {
            ...
            {
                YellowSquare_sprite.SetPosition({static_cast<float>(i * CFDCP), static_cast<float>(j * CFDCP)});
                YellowSquare_sprite.Draw(window);
            }
            break;
        }
        ...
        {
            DarkCircle_sprite.SetPosition({static_cast<float>(i * CFDCP), static_cast<float>(j * CFDCP)});
            DarkCircle_sprite.Draw(window);
        }
    }
...
void BacklightOfPossibleKnightMovements(AppState& appState, int OpponentColor, int ox, int oy)
{
    ...
    for (j = -2; j < 3; j++)
    {
        ...
        for (l= 0; l < 2; l++)
        {
            ...
            {
                DarkCircle_sprite.SetPosition({static_cast<float>((ox + i) * CFDCP), static_cast<float>((oy + j) * CFDCP)});
                DarkCircle_sprite.Draw(window);
            }
            ...
            {
                YellowSquare_sprite.SetPosition({static_cast<float>((ox + i) * CFDCP), static_cast<float>((oy + j) * CFDCP)});
                YellowSquare_sprite.Draw(window);
            }
            i = -i;
        }
...
, oy, rox, oy);
            ...
            {
                LightCircle_sprite.SetPosition({static_cast<float>((ox - 2) * CFDCP), static_cast<float>(oy * CFDCP)});
...
void BacklightOfKingUnderShah(AppState& appState, int ox, int oy)
{
    ...
    if (kingx != 0 && kingy != 0)
    {
        RedSquare_sprite.SetPosition({static_cast<float>(kingx * CFDCP), static_cast<float>(kingy * CFDCP)});
